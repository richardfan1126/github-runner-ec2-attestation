...
## Decision 11 — Grouped-by-category summary layout (FR-030 / SC-007, Clarifications 2026-06-14)

**Decision**: Render the build-summary configuration as **labeled per-category subsections** rather than one flat table. `print_config.py` defines a module-level *ordered* map `CONFIG_CATEGORIES: dict[str, list[str]]` from a category label (e.g. `HTTP Server`, `Execution`, `Rate Limiting`, `Storage`, `NitroTPM`, `OIDC Authentication`, `Container Execution`, `Container Security`) to the ordered list of `ServerConfig` field names in that category. For each category in map order it emits a `####` heading followed by that category's own `| Setting | Value |` table. Any `dataclasses.fields(ServerConfig)` field **not** present in any category list is collected into a catch-all **"Other"** subsection rendered **last** (only when non-empty). Subsection order = map order ("Other" last); within-subsection order = the map's listed order for that category.

**Rationale**: Grouping related settings makes a relaxed security setting easy to locate during audit (the eight container-security settings sit in one labeled block) and matches the eight categories FR-030 already enumerates. The catch-all "Other" group is what keeps grouping **drift-proof**: the printed field set is still derived from `dataclasses.fields(ServerConfig)`, so a newly added field is never dropped — it surfaces under "Other" until a maintainer assigns it a category. This satisfies the spec's "every effective setting, superset of `.env.example`, cannot drift" guarantee (SC-007) without the fragility of a build-failing assertion. Each field appears exactly once (mapped fields in their category, the rest in "Other").

**Alternatives considered**: (a) *One flat table* — rejected by clarification (no grouping). (b) *Single table with a "Category" column* — rejected: less scannable than per-category subsections. (c) *Maintained map + build failure if any field is uncategorized* — considered (stronger forcing function) but rejected by clarification in favor of the non-breaking catch-all "Other" group, so a new field never breaks the build. (d) *Refactor `ServerConfig` into nested per-category sub-objects to derive grouping structurally* — rejected: a much larger code change to a flat dataclass that other modules already consume field-by-field.
