...
FR-030 introduces **no new entity**. The build-workflow summary serializes the existing `ServerConfig` entity in full: `.github/scripts/print_config.py` (build tooling, outside `src/`) resolves the AMI's baked-in env file through `load_config()` and renders **every** `dataclasses.fields(ServerConfig)` field (a superset of the `.env.example` keys, including the eight security settings above). Fields absent from the env file resolve to their dataclass defaults; values are printed verbatim. Because the row set is read from the dataclass, the snapshot cannot drift from the schema (SC-007).

The fields are presented **grouped by configuration category** into labeled subsections (Clarifications 2026-06-14; research Decision 11), not a single flat table. Grouping is a presentation-only concern backed by an ordered `CONFIG_CATEGORIES` map (category label → ordered `ServerConfig` field names) defined in the helper. Each category renders as a heading + its own `| Setting | Value |` table in map order; any field not assigned to a category falls into a catch-all **"Other"** subsection rendered last. The grouping carries no schema meaning — it does not alter `ServerConfig`; every field still appears exactly once, so the catch-all keeps the snapshot drift-proof even before a new field is categorized.
