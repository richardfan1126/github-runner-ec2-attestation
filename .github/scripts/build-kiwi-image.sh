...
#
# Flavor model (execution-build-images, task 2.1/2.3/4.1):
#   CONTAINER_IMAGE and CONTAINER_IMAGE_DIGEST are no longer read from a committed
#   env file. They are pipeline-injected into the effective env after the flavor's
#   Dockerfile is built and pushed to GHCR.  The effective env file at
#   EXECUTOR_ENV_FILE is generated by the merge step (task 2.1) and carries the
#   injected digest (task 2.3) before this bake step runs.
echo ""
...
if [ ! -f "${EXECUTOR_ENV_FILE}" ]; then
    ...
    echo "::error::  The effective env is generated by the flavor merge step (task 2.1)."
    echo "::error::  Ensure build-kiwi-image.sh is called after the merge produces this file."
    exit 1
fi

# Read CONTAINER_IMAGE / CONTAINER_IMAGE_DIGEST from the effective env file.
# In the flavor model these values are pipeline-injected (task 2.3/4.1) and
# present in the generated effective env, not in any committed source file.
