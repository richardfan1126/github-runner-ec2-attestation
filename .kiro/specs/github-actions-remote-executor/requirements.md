...
- **OIDC_Token**: JSON Web Token (JWT) issued by GitHub's OIDC provider (`https://token.actions.githubusercontent.com`) to a GitHub Actions workflow, included in the encrypted request body as the `oidc_token` field to authenticate /execute requests. The /execution/{id}/output endpoint does not require an OIDC token; possession of the execution-bound Shared_Key serves as authentication for that endpoint.
...
2. THE /execution/{id}/output endpoint SHALL NOT require an oidc_token field in the decrypted request body; possession of the execution-bound Shared_Key (from the PQ_Hybrid_KEM exchange during /execute) serves as the authentication mechanism
3. IF the oidc_token field is missing from the decrypted request body of a /execute request, THEN THE Request_Validator SHALL reject the request with HTTP 401 Unauthorized and an error message indicating the token is required
...
1. THE GHA_Server SHALL provide an HTTP POST endpoint for retrieving execution results (POST is required because the request body contains an encrypted payload)
2. THE GHA_Server SHALL accept the Execution_ID as a URL parameter
3. THE GHA_Server SHALL authenticate /execution/{id}/output requests solely by verifying that the client possesses the execution-bound Shared_Key (derived during the PQ_Hybrid_KEM exchange on /execute); no separate OIDC token validation is required on this endpoint
...
19. THE Build_Workflow SHALL install Python dependencies into the KIWI image using a lockfile-enforced installation path: either `uv sync --frozen` (which installs exactly the versions in `uv.lock`), a hash-checked export from `uv.lock` (via `uv export --frozen --format requirements-txt --no-dev`, which includes hashes by default), or `pip install --require-hashes` against the exported requirements; the installation SHALL NOT use version ranges from `pyproject.toml` directly
...
2. WHEN the client sends a request to /execution/{execution_id}/output, THE client SHALL encrypt the request payload (containing the optional nonce and offset) using the Shared_Key
