...
Python dependencies are split between `pyproject.toml` (remote executor: fastapi, uvicorn, requests, docker, cryptography, wolfcrypt-py, PyJWT) and `scripts/pyproject.toml` (build scripts: boto3, paramiko). Only remote executor deps are installed in the KIWI image via a two-phase process: download wheels with hash verification from `uv.lock` (build-kiwi-image.sh uses `uv export --frozen` + `pip3 download --require-hashes`) → offline install from wheels with `pip install --require-hashes` (config.sh). Version ranges from `pyproject.toml` are NOT used for AMI-embedded dependencies. See Requirement 12 and the Overview section for details.
...
*For any* KIWI image build, the `appliance.kiwi` package definition should include the `docker` package (available in AL2023 core repos), plus runtime library dependencies (`fuse3`, `libseccomp`, `glib2`, `libcap`). The `uidmap` package should NOT be listed separately because `shadow-utils` (which provides `newuidmap`/`newgidmap`) is already included via the AL2023 core collection. The `libslirp` package should NOT be listed because it is not available in AL2023 and is instead compiled from source and shipped as a shared library in `/usr/local/lib64/`. The `rootlesskit`, `slirp4netns`, and `fuse-overlayfs` binaries should NOT be listed as DNF packages (they are not available in AL2023) but should instead be compiled from source by the build script and placed at `/usr/local/bin/` in the KIWI image overlay. The runtime image should NOT include packages without documented build-time or runtime justification (`awscli` and `pciutils` must be absent); `git` is retained with explicit justification for Repository_Client operations; `binutils` is retained with explicit justification — it is required by `dracut --uefi` for UKI assembly (`objcopy`) during the KIWI create step and cannot be removed at any point during the build; `python3.11-pip` is present in `<packages type="image">` but removed via `<packages type="uninstall">` (build-time only).

Packages that are required during `config.sh` execution but have no runtime justification should be listed in both `<packages type="image">` (so they are installed) and `<packages type="uninstall">` (so KIWI removes them after `config.sh` completes). This is the documented KIWI mechanism for build-time-only dependencies — the uninstall phase runs after `config.sh` in the prepare step, gracefully removing the packages and their orphaned dependencies from the final image. Specifically, `python3.11-pip` is required during `config.sh` for `pip3.11 install --no-index` of pre-downloaded wheels, but has no runtime purpose and must be absent from the final image.

`binutils` is required during the KIWI **create step** because `dracut --uefi` (invoked via `chroot` into the image root by KIWI's `create_uki()`) uses `objcopy` to assemble the Unified Kernel Image (UKI). The KIWI build lifecycle makes it impossible to remove `binutils` before the final image: `<packages type="uninstall">` runs during the prepare step (before the create step), and `pre_disk_sync.sh` runs **before** dracut/UKI generation. There is no KIWI hook that runs after UKI assembly but before the root tree is written to disk. Therefore, `binutils` must remain in the final runtime image as a necessary build infrastructure dependency.
