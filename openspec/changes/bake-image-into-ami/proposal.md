...
**Design-time spike (the residual C1-a risk)**: for any loader that *converts* the layout (`skopeo copy oci:→docker-daemon:`, or docker-archive + `docker load`), confirm on the rootless `fuse-overlayfs` daemon that it produces a local image whose ID equals the config digest read from the verified manifest, for **both** an OCI-media-type and a docker-media-type source image. The transient-`127.0.0.1`-registry + `docker pull @digest` path needs **no spike** — a real pull is faithful by construction, re-verifies the manifest digest natively (satisfying the hard requirement above as a side effect), and even preserves `RepoDigests` (Docker auto-treats `127.0.0.0/8` as insecure-allowed, so no `daemon.json` change) — which is why it is the leading candidate. All candidate paths bake the same deterministic OCI layout and differ only in the runtime import step; `design.md` holds the full loader comparison.
