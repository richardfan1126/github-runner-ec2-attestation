...
The Attestation_Generator SHALL emit a claims document (`claims_raw`) carrying a top-level `schema_version` and the claim fields removed from the inline layout — at minimum `repository_url`, `commit_hash`, `script_path`, `script_env_hash`, and the `security` posture block for execution claims. `schema_version` is distinct from the envelope's `v`: `v` versions the envelope format, `schema_version` versions the claims content, and they evolve independently.

#### Scenario: Claims document carries schema_version and moved fields

- **WHEN** an execution attestation is generated
- **THEN** `claims_raw` contains `schema_version` and the execution claim fields (`repository_url`, `commit_hash`, `script_path`, `script_env_hash`, `security`), and `schema_version` identifies the claims content format independently of the envelope `v`

### Requirement: Digest binding over transmitted bytes

The binding `claims_digest = sha256(claims_raw)` SHALL be computed over the exact bytes transmitted in the response body. A verifier SHALL check the binding by hashing the bytes it received and comparing to `claims_digest`; it SHALL NOT re-serialize or re-canonicalize the claims document to check the binding. Digest values SHALL be written with an algorithm prefix (`sha256:<hex>`), and verifiers SHALL select the hash by prefix and reject unknown algorithms.

#### Scenario: Binding checked against received bytes

- **WHEN** a verifier receives `claims_raw` and the signed `claims_digest`
- **THEN** hashing the received `claims_raw` bytes with the algorithm named by the `sha256:` prefix reproduces `claims_digest` without any canonicalization step
...
#### Scenario: Unknown schema_version rejected before reading

- **WHEN** `claims_raw` carries a `schema_version` the verifier does not recognize
- **THEN** the verifier rejects the attestation before interpreting any claim field
