# execution-build-images Specification

## Purpose

Define and build multiple per-flavor execution-container images and AMIs, one per build environment variant ("flavor"). Each flavor is a co-located directory under `flavors/` containing a `Dockerfile` and an `env` configuration file. Flavors share a common base AMI (the KIWI image) and configuration schema (the executor's `ServerConfig` keys) but produce distinct execution-container images, distinct effective sandbox configurations baked into the verity root, and distinct AMIs with distinct PCR4 values. A `detect-changes` job maps changed paths to a selective rebuild matrix, and `flavors.lock` records the deployed state of all flavors.
