# attestation-claims Specification

## Purpose

Define the claims-document envelope format carried in the NitroTPM attestation's `user_data`: the compact signed envelope (`{ v, claims_digest, timestamp, execution_id }`), the versioned claims schema and its canonicalization/digest rules, the hash-the-transmitted-bytes binding between the envelope and the claims document, the strict envelope/claims field partition (inline envelope fields are never repeated in the claims document), and the mandatory verifier binding-check contract (recompute digest → reject unknown version → read fields). This capability is agnostic about which specific claims are carried — see `gpu-attestation` and `remote-executor` for the claims themselves.
