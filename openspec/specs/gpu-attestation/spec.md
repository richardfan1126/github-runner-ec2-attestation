# gpu-attestation Specification

## Purpose

Define the GPU information carried in the attestation: the `gpu` claims block and its fields, sourced from the PCR4-measured NVIDIA driver via NVML (and the resulting trust semantics — a measured-driver self-report, not hardware attestation), per-device array handling for multi-GPU instances, the `{ enabled: false }` shape when GPU is disabled, the reserved nested slot (`gpu.attestation.report_digest`) for a future NVIDIA NRAS hardware-attestation report, and the supported-instance bound (NitroTPM attestation exists only on specific virtualized accelerated instances — not on P4d/P4de (A100), P3 (V100), or bare-metal).
